...
<div class="breadcrumb-container">
    <?php
    $category = isset($_GET['article_category']) ? sanitize_text_field($_GET['article_category']) : '';
    $industry = isset($_GET['article_industry']) ? sanitize_text_field($_GET['article_industry']) : '';
    ...
    if ($category) {
        $category_term = get_term_by('slug', $category, 'article_category');
        if ($category_term) {
            $breadcrumbs .= ' > <a href="' . esc_url(home_url('?article_category=' . $category)) . '">' . esc_html($category_term->name) . '</a>';
        }
    }

    if ($industry) {
        $industry_term = get_term_by('slug', $industry, 'article_industry');
        if ($industry_term) {
            $breadcrumbs .= ' > <a href="' . esc_url(home_url('?article_industry=' . $industry)) . '">' . esc_html($industry_term->name) . '</a>';
        }
    }
    ...
    <div class="grid-item post-card mt-24">
        <a href="<?php the_permalink(); ?>" class="post-card-link" aria-label="Read more about <?php the_title_attribute(); ?>">
                ...
                <h2 class="post-para"><?php the_field('article_heading'); ?></h2>
