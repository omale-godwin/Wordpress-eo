jQuery(document).ready(function ($) {
    ...
    function fetchPosts() {
        ...
        if (!filters.category_slug && !filters.industry_slug && !filters.search) {
            return; // No filters selected, skip
        }
        ...
        $.ajax({
            ...
            data: filters,
            beforeSend: function () {
                $("#search-filtered-posts").html("<p class='white'>Loading...</p>");
            },
    ...
    // Trigger fetch on dropdown or input change
    $("#search-category-dropdown, #search-industry-dropdown").on("change", fetchPosts);
    $("#search-input").on("input", fetchPosts);

    // Optional: Tag clicking behavior
