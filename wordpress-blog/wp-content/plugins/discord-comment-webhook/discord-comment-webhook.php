<?php
/*
Plugin Name: Discord Comment Webhook - Blog (Instant Notify)
Description: Sends ALL new comments instantly to Discord (no approval required)
Version: 1.2
*/

add_action('comment_post', 'eo_discord_instant_comment_notify_blog', 10, 2);

function eo_discord_instant_comment_notify_blog($comment_ID, $comment_approved) {

    $comment = get_comment($comment_ID);
    eo_send_discord_comment_instant_blog($comment);
}

function eo_send_discord_comment_instant_blog($comment) {

    // 👉 Use BLOG Discord Webhook URL here
    ...
    $message = [
        "content" => "**New Blog Comment Submitted!**",
        "embeds" => [[
            "title" => $post_title,
            "url" => $post_link,
            "color" => hexdec("5865F2"), // Discord blue
            "fields" => [
                ["name" => "Source", "value" => "Blog", "inline" => true],
    ...
    if (is_wp_error($response)) {
        error_log("Discord Webhook Error (BLOG): " . $response->get_error_message());
    }
}