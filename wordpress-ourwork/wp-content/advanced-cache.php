...
// WP-Optimize advanced-cache.php (written by version: 4.4.0) (homeurl: https://ourwork.electricoctopus.agency/) (abspath: /opt/bitnami/wordpress/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/bitnami/wordpress/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://ourwork.electricoctopus.agency/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/bitnami/wordpress/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-ourwork.electricoctopus.agency.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-ourwork.electricoctopus.agency.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-ourwork.electricoctopus.agency.php');
}
